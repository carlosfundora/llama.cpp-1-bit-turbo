...
int main(int argc, char ** argv) {
    ...
    std::thread worker([&]() {
        ...
        while (work_queue.pop(item)) {
            ...
            size_t audio_read_head = 0;
            auto flush_audio_chunk = [&](size_t size_to_flush) {
                if (audio_read_head >= audio_buffer.size() || size_to_flush == 0) return;
                size_t actual_flush = std::min(size_to_flush, audio_buffer.size() - audio_read_head);
                std::string audio_base64 =
                    base64::encode(reinterpret_cast<const char *>(audio_buffer.data() + audio_read_head), actual_flush * sizeof(audio_buffer.front()));
                json chunk = {
                    { "object",  "chat.completion.chunk"                                                           },
                    { "created", std::time(0)                                                                      },
                ...
                output->push("data: " + chunk.dump() + "\n\n");
                audio_read_head += actual_flush;
            };

            auto audio_cb = [&output, &item, &audio_buffer, &audio_read_head, &flush_audio_chunk](const std::vector<int16_t> & audio) {
                item.check_abort();
                if (output->aborted.load()) {
                    return;
                }
                audio_buffer.insert(audio_buffer.end(), audio.begin(), audio.end());
                while (audio_buffer.size() - audio_read_head >= 480) {
                    flush_audio_chunk(480);
                }
                if (audio_read_head >= 4800) {
                    audio_buffer.erase(audio_buffer.begin(), audio_buffer.begin() + audio_read_head);
                    audio_read_head = 0;
                }
            };
            ...
            flush_audio_chunk(audio_buffer.size() - audio_read_head);
