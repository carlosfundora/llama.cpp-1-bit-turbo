...
int main(int argc, char ** argv) {
    ...
    std::thread worker([&]() {
        ...
        while (work_queue.pop(item)) {
            ...
            const size_t MAX_BUFFER_SIZE = 4800; // threshold for periodic cleanup
            auto flush_audio_chunk = [&](size_t size_to_flush) {
                if (audio_buffer.size() <= audio_buffer_head || size_to_flush == 0) return;
                size_t available = audio_buffer.size() - audio_buffer_head;
            ...
            auto audio_cb = [&output, &item, &audio_buffer, &audio_buffer_head, &flush_audio_chunk, &MAX_BUFFER_SIZE](const std::vector<int16_t> & audio) {
                item.check_abort();
                if (output->aborted.load()) {
                    return;
                }
                ...
                if (audio_buffer_head > MAX_BUFFER_SIZE) {
