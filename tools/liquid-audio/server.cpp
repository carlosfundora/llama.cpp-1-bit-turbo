    ...
    std::thread worker([&]() {
        ...
        while (work_queue.pop(item)) {
            ...
            auto flush_audio_chunk = [&](size_t size_to_flush) {
                ...
                size_t actual_flush = std::min(size_to_flush, audio_buffer.size() - audio_buffer_offset);
                if (actual_flush == 0) return;
                ...
                // Periodically prune the buffer to prevent memory bloat
