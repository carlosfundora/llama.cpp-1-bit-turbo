...
int main() {
    ...
    std::vector<int32_t> pos(100000, -1);
    ...
    for (uint32_t i = 0; i < 50000; i++) {
        ...
        if (count == 0) {
            ...
            if (i == used_min) {
                while (used_min < 100000 && pos[used_min] == -1) used_min++;
            }
            if (i + 1 == used_max_p1) {
                while (used_max_p1 > 0 && pos[used_max_p1 - 1] == -1) used_max_p1--;
            }
            volatile uint32_t min = used_min;
            volatile uint32_t max = used_max_p1;
        }
    ...
    std::cout << "pos min/max: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << " us" << std::endl;
