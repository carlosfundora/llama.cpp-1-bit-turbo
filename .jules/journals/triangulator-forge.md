...
Action: For our engine, ensure any HTTP or external interface routing strictly adheres to the 'Ollama pattern' of abstracting the backend engine behind a narrow API boundary, and avoid the vLLM pitfall of heavily coupling web frameworks (FastAPI) and orchestration (Ray) with tensor execution logic.
 ...
 2024-05-01 - Python Framework Entanglement vs C++ Isolation
Learning: Deeply integrating execution logic with frameworks like Ray (vLLM) or PyTorch (SGLang) heavily degrades the transportability of advanced architectural features. SGLang’s RadixAttention, while conceptually brilliant (Radix Tree based KV cache), is deeply coupled to Python orchestration (`sglang.srt.mem_cache.radix_cache.TreeNode`) and PyTorch tensor allocations. Similarly, vLLM’s engine tightly couples its scheduling with Ray actor distributed coordination. Conversely, Ollama demonstrates a clean abstraction by putting API routing (`server/routes.go`) over a strict C++ runner interface (`llm.LlamaServer`), achieving superior DX at the cost of execution granularity.
Action: When extracting features like Radix Tree KV allocation for llama.cpp, avoid importing orchestration boundaries. Re-implement the pure data structure directly in C++ (like `llama_kv_cache`), avoiding tight coupling to Python lifecycle management or web framework layers.
